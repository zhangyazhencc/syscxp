 ...
 * @Description: .
 */
public class L3NetworkMonitorManagerImpl extends AbstractService implements L3NetworkMonitorManager, Component, ApiMessageInterceptor {
    ...
    private void handle(APIConfigL3NetworkMonitorMsg msg) {
        ...
        List<L3NetworkMonitorVO> l3NetworkMonitorVOS = new ArrayList<L3NetworkMonitorVO>();
        ...
        if (StringUtils.isNotEmpty(msg.getMonitorIp())) {
            if (StringUtils.isEmpty(l3EndpointVO.getMonitorIp())) {
                ...
                l3NetworkMonitor.startMonitor(l3EndpointVO, l3NetworkMonitorVOS);
            } else {
                if (StringUtils.equals(msg.getMonitorIp(), l3EndpointVO.getMonitorIp())) {
                    // 更新本端出的监控机监控
                    l3NetworkMonitor.updateSrcMonitor(l3EndpointVO, l3NetworkMonitorVOS);
                } else {
                    // 更新对端监控机监控
                    l3EndpointVO.setMonitorIp(msg.getMonitorIp());
                    l3NetworkMonitor.updateDstMonitor(l3EndpointVO, l3NetworkMonitorVOS);
                }
            }
        } else {
            if (StringUtils.isNotEmpty(l3EndpointVO.getMonitorIp())) {
                // 删除监控ip，停止监控
                l3EndpointVO.setMonitorIp(msg.getMonitorIp());
                l3NetworkMonitor.stopMonitor(l3EndpointVO, l3NetworkMonitorVOS);
            }
        }
