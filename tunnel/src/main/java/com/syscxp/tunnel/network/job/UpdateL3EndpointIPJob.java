...
import com.syscxp.header.tunnel.network.UpdateL3EndpointIPMsg;
...
@RestartableJob
@UniqueResourceJob
public class UpdateL3EndpointIPJob implements Job {
    private static final CLogger logger = Utils.getLogger(UpdateL3EndpointIPJob.class);
    ...
    public void run(ReturnValueCompletion<Object> completion) {
        ...
        try {
            if(dbf.isExist(l3EndpointUuid, L3EndPointVO.class)){
                logger.info("开始执行修改L3互联IP JOB【控制器下发修改L3互联IP】");

                L3EndPointVO vo = dbf.findByUuid(l3EndpointUuid,L3EndPointVO.class);
                //创建任务
                TaskResourceVO taskResourceVO = new L3NetworkBase().newTaskResourceVO(vo, TaskType.UpdateL3EndpointIP);
