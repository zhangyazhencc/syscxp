...
@RestartableJob
@UniqueResourceJob
public class DeleteL3EndpointJob implements Job {
    private static final CLogger logger = Utils.getLogger(DeleteL3EndpointJob.class);
    ...
    public void run(ReturnValueCompletion<Object> completion) {
        ...
        try {
            if(dbf.isExist(l3EndpointUuid, L3EndPointVO.class)){
                logger.info("开始执行删除L3连接点 JOB【控制器下发删除L3Endpoint】");
