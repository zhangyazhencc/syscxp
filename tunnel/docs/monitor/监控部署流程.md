...
+ 开启监控（使用原监控ip）
    ...
    - 增加check_result，endpointA_switch_port_id,endpointB_switch_port_id 栏位
            ...
            ALTER TABLE tunnel_icmp ADD COLUMN `endpointA_switch_port_id` VARCHAR(32);
            ALTER TABLE tunnel_icmp ADD COLUMN `endpointB_switch_port_id` VARCHAR(32);
            ALTER TABLE tunnel_icmp ADD COLUMN `check_result` VARCHAR(256);
...
+ 告警策略、告警规则、策略规则绑定、策略资源绑定
    - 数据整理
	    ...
	    - 新增 comparison_rule_uuid, monitor_target_uuid, policy_uuid, reguration_uuid 字段
	         ...
	         ALTER TABLE tunnel_strategy ADD (`comparison_rule_uuid` varchar(32));
             ALTER TABLE tunnel_strategy ADD (`monitor_target_uuid` varchar(32));
             ALTER TABLE tunnel_strategy ADD (`policy_uuid` varchar(32));
             ALTER TABLE tunnel_strategy ADD (`reguration_uuid` varchar(32));
