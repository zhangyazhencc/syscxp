...
public class BalanceManagerImpl extends AbstractService implements ApiMessageInterceptor {
    ...
    private void handle(APIGetExpenseGrossMonthMsg msg) {
        ...
        String sql = "select DATE_FORMAT(payTime,'%Y-%m') mon,sum(payPresent)+sum(payCash) as payTotal from OrderVO where accountUuid = :accountUuid and state = 'PAID' and DATE_FORMAT(payTime,'%Y-%m-%d  %T') between :dateStart and :dateEnd and type = :type group by mon order by mon asc";
        ...
        q.setParameter("type", DealType.DEDUCTION);
    ...
    private void handle(APIRechargeMsg msg) {
        ...
        new DealDetailVOHelper(dbf).saveDealDetailVO(accountUuid, DealWay.CASH_BILL, total, BigDecimal.ZERO, currentTimestamp, DealType.RECHARGE, DealState.FAILURE, accountBalanceVO.getCashBalance().add(total), outTradeNO, outTradeNO, msg.getSession().getAccountUuid(),null);
