...
CREATE DEFINER =`root`@`%` PROCEDURE `generateOrder`()
  BEGIN
    ...
    DECLARE generateOrder CURSOR FOR SELECT
                                       ...
                                       productChargeModel,
                                       expireDate
                                     FROM syscxp_tunnel.`TunnelEO`
                                     WHERE accountUuid IS NOT NULL AND deleted IS NULL
                                     ORDER BY TYPE;
    ...
    REPEAT
      FETCH generateOrder
      INTO pd, acUuid, pn, bandw, bw, category, dura, pcm, exd;
      ...
      BEGIN
        ...
        DECLARE disc INT;
        DECLARE sport CURSOR FOR SELECT
                                   endpointUuid,
                                   switchPortUuid,
                                   sortTag
                                 FROM syscxp_tunnel.TunnelSwitchPortVO
                                 WHERE tunnelUuid = pd AND (sTag = 'A' OR sTag = 'Z');
        DECLARE EXIT HANDLER FOR NOT FOUND SET ef := 0;
        OPEN sport;
        REPEAT
          FETCH sport
          INTO epUuid, sportU, sTag;
          SELECT
            portType,
            switchUuid
          INTO pt, sUid
          FROM syscxp_tunnel.`SwitchPortVO`
          WHERE `uuid` = sportU;
          IF (pt = 'SHARE')
          THEN
            SET description = CONCAT_WS('', CONCAT_WS('-', CONCAT_WS('', description, '{"name":"端口'), sTag),
                                        '","value":"共享端口"},');
            SELECT endpointUuid
            INTO eUid
            FROM syscxp_tunnel.`SwitchEO`
            WHERE `uuid` = sUid;
            IF (bw < 524288000)
            THEN
              SET concod = 'LT500M';
            ELSEIF (bw >= 1073741824 * 2)
              THEN
                SET concod = 'GT2G';
            ELSE
            ...
            SELECT
              ...
              unitPrice
            INTO connam, unitp
            FROM syscxp_billing.`ProductPriceUnitVO`
            WHERE productCategoryUuid = 'SHARE' AND lineCode = eUid AND configCode = concod;
            ...
            SET priceInfo = CONCAT_WS('', CONCAT_WS('', CONCAT_WS('', priceInfo, '{"configName":"'),
                                                    CONCAT_WS('', connam, '","originalPrice":')),
                                      CONCAT_WS('', CONCAT_WS('', unitp, ',"realPayPrice":'),
                                                CONCAT_WS('', unitp, ',"discount":100},')));
          ...
          SELECT `name`
          INTO eNam
          FROM syscxp_tunnel.`EndpointEO`
          WHERE `uuid` = epUuid;
          SET description = CONCAT_WS('', CONCAT_WS('-', CONCAT_WS('', description, '{"name":"连接点'), sTag),
                                      CONCAT_WS('', '","value":",', CONCAT_WS('', eNam, '"},')));
      ...
      THEN
        ...
        INTO connam, unitp
        FROM syscxp_billing.`ProductPriceUnitVO`
        WHERE productCategoryUuid = 'CITY' AND configCode = bandw;
        SET totpri = totpri + unitp;
        SET priceInfo = CONCAT_WS('', CONCAT_WS('', CONCAT_WS('', priceInfo, '{"configName":"'),
                                                CONCAT_WS('', connam, '","originalPrice":')),
                                  CONCAT_WS('', CONCAT_WS('', unitp, ',"realPayPrice":'),
                                            CONCAT_WS('', unitp, ',"discount":100},')));
      ELSEIF (category = 'LONG')
        THEN
          SELECT
            configName,
            unitPrice
          INTO connam, unitp
          FROM syscxp_billing.`ProductPriceUnitVO`
          WHERE productCategoryUuid = 'LONG' AND configCode = bandw;
          SET totpri = totpri + unitp;
          SET priceInfo = CONCAT_WS('', CONCAT_WS('', CONCAT_WS('', priceInfo, '{"configName":"'),
                                                  CONCAT_WS('', connam, '","originalPrice":')),
                                    CONCAT_WS('', CONCAT_WS('', unitp, ',"realPayPrice":'),
      ...
      ELSEIF (category = 'REGION')
        THEN
          ...
          INTO connam, unitp
          FROM syscxp_billing.`ProductPriceUnitVO`
          WHERE productCategoryUuid = 'REGION' AND areaCode = (SELECT zoneUuid
                                                               FROM syscxp_tunnel.`ZoneNodeRefVO`
                                                               WHERE nodeUuid = (SELECT nodeUuid
                                                                                 FROM syscxp_tunnel.`EndpointEO`
                                                                                 WHERE `uuid` = (SELECT endpointUuid
                                                                                                 FROM
                                                                                                   syscxp_tunnel.`TunnelSwitchPortVO`
                                                                                                 WHERE
                                                                                                   tunnelUuid = pd AND
                                                                                                   sortTag = 'A'))) AND
                configCode = bandw;
      ...
      ELSEIF (category = 'CHINA1ABROAD')
        THEN
          SELECT REPLACE(GROUP_CONCAT(CASE WHEN country = 'CHINA'
            THEN city
                                      ELSE country END), ',', '/')
          INTO lcode
          FROM syscxp_tunnel.`NodeEO`
          WHERE `uuid` IN (SELECT nodeUuid
                           FROM syscxp_tunnel.`EndpointEO`
                           WHERE `uuid` IN (SELECT endpointUuid
                                            FROM syscxp_tunnel.`TunnelSwitchPortVO`
                                            WHERE tunnelUuid = pd));
          SELECT COUNT(1)
          INTO cou
          FROM syscxp_billing.`ProductPriceUnitVO`
          WHERE
            productCategoryUuid = 'ABROAD' AND areaCode = 'CHINA2ABROAD' AND lineCode = lcode AND configCode = bandw;
          ...
          INTO connam, unitp
          FROM syscxp_billing.`ProductPriceUnitVO`
          WHERE productCategoryUuid = 'ABROAD' AND areaCode = 'CHINA2ABROAD'
                AND lineCode = lcode
      ...
      ELSEIF (category = 'CHINA2ABROAD')
        THEN
          ...
          WHERE `uuid` IN (SELECT nodeUuid
                           ...
                           WHERE `uuid` IN (SELECT endpointUuid
                                            FROM syscxp_tunnel.`TunnelSwitchPortVO`
                                            WHERE tunnelUuid = pd AND sortTag = 'A'));
          IF (countri = 'CHINA')
          THEN
            SELECT city
            INTO cityA
            ...
            WHERE `uuid` IN (SELECT nodeUuid
                             ...
                             WHERE `uuid` IN (SELECT endpointUuid
                                              FROM syscxp_tunnel.`TunnelSwitchPortVO`
                                              WHERE tunnelUuid = pd AND sortTag = 'B'));
            SELECT COUNT(1)
            INTO couA
            FROM syscxp_tunnel.`ZoneNodeRefVO`
            WHERE nodeUuid = (SELECT nodeUuid
            ...
            ELSEIF (zoneA = zoneB)
              THEN
                ...
                INTO connam, unitp
                FROM syscxp_billing.`ProductPriceUnitVO`
                WHERE productCategoryUuid = 'REGION' AND areaCode = zoneA AND configCode = bandw;
                SET totpri = totpri + unitp;
                SET priceInfo = CONCAT_WS('', CONCAT_WS('', CONCAT_WS('', priceInfo, '{"configName":"'),
                                                        CONCAT_WS('', connam, '","originalPrice":')),
                                          CONCAT_WS('', CONCAT_WS('', unitp, ',"realPayPrice":'),
            ...
            WHERE `uuid` IN (SELECT nodeUuid
                             ...
                             WHERE `uuid` IN (SELECT endpointUuid
                                              FROM syscxp_tunnel.`TunnelSwitchPortVO`
                                              WHERE tunnelUuid = pd AND (sortTag = 'B' OR sortTag = 'Z')));
            SELECT COUNT(1)
            INTO cou
            FROM syscxp_billing.`ProductPriceUnitVO`
            WHERE
          ...
          ELSE
            ...
            WHERE `uuid` IN (SELECT nodeUuid
                             ...
                             WHERE `uuid` IN (SELECT endpointUuid
                                              FROM syscxp_tunnel.`TunnelSwitchPortVO`
                                              WHERE tunnelUuid = pd AND sortTag = 'Z'));
            SELECT city
            INTO cityB
            FROM syscxp_tunnel.`NodeEO`
            WHERE `uuid` IN (SELECT nodeUuid
            ...
            WHERE `uuid` IN (SELECT nodeUuid
                             ...
                             WHERE `uuid` IN (SELECT endpointUuid
                                              FROM syscxp_tunnel.`TunnelSwitchPortVO`
                                              WHERE tunnelUuid = pd AND (sortTag = 'B' OR sortTag = 'A')));
            SELECT COUNT(1)
            INTO cou
            FROM syscxp_billing.`ProductPriceUnitVO`
            WHERE
      ...
      ELSE
        ...
        SELECT REPLACE(GROUP_CONCAT(country), ',', '/')
        INTO lcode
        FROM syscxp_tunnel.`NodeEO`
        WHERE `uuid` IN (SELECT nodeUuid
                         FROM syscxp_tunnel.`EndpointEO`
        ...
        INTO cou
        FROM syscxp_billing.`ProductPriceUnitVO`
        WHERE productCategoryUuid = 'ABROAD' AND areaCode = 'CHINA2ABROAD' AND lineCode = lcode AND configCode = bandw;
      ...
      SET description = CONCAT_WS('', CONCAT_WS('', description, '{"name":"带宽","value":"'),
                                  CONCAT_WS('', bandw, '"}]}'));
