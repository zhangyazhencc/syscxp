...
BEGIN
     ...
     BEGIN
       ...
       DECLARE disc INT;
       DECLARE sport CURSOR FOR  SELECT endpointUuid,switchPortUuid,sortTag FROM syscxp_tunnel.TunnelSwitchPortVO WHERE tunnelUuid = pd AND (sTag='A' OR sTag='Z');
	...
	SET priceInfo=CONCAT_WS('',CONCAT_WS('',CONCAT_WS('',priceInfo,'{"configName":"'),CONCAT_WS('',connam,'","originalPrice":')),CONCAT_WS('',CONCAT_WS('',unitp,',"realPayPrice":'),CONCAT_WS('',unitp,',"discount":100},')));
      ...
      ELSEIF (category='CHINA1ABROAD') THEN
        SELECT  REPLACE(GROUP_CONCAT(CASE WHEN country='CHINA' THEN city ELSE country END),',','/') INTO lcode FROM syscxp_tunnel.`NodeEO` WHERE `uuid` IN( SELECT nodeUuid FROM syscxp_tunnel.`EndpointEO` WHERE `uuid` IN (SELECT endpointUuid FROM syscxp_tunnel.`TunnelSwitchPortVO` WHERE tunnelUuid =pd));
