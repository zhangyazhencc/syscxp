...
import com.syscxp.header.search.*;
...
import com.syscxp.core.db.TransactionalCallback;
...
public class InventoryIndexManagerImpl extends AbstractService implements InventoryIndexManager, TransactionalCallback {
        ...
        if (!msg.getInventoryNames().isEmpty()) {
            ...
            for (String invname : msg.getInventoryNames()) {
                if (!validInventoryNames.contains(invname)) {
                    //ErrorCodeFacade.setErrorToApiEvent(ErrorCodeFacade.BuiltinErrors.INVALID_ARGRUMENT.toString(), String.mediaType("syscxp doesn't have this inventory[%s]", invname), evt);
                    bus.publish(evt);
                    return;
                }
            }
    ...
    private void doGenerateSqlTriggerText(SqlTrigger trigger, Class<?> clazz, StringBuilder sb) {
        ...
        sb.append(String.format("\nDROP TRIGGER IF EXISTS `syscxp`.%s;", insertTriggerName));
    ...
    private void handle(APISearchGenerateSqlTriggerMsg msg) {
        ...
        if (resultPath == null) {
            resultPath = PathUtil.join(System.getProperty("user.home"), "syscxp-sql-trigger.sql");
        }
