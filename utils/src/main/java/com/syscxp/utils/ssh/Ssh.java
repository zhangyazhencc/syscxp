...
public class Ssh {
    ...
    private class ScriptRunner {
        ...
        ScriptRunner(String scriptName, String parameters, Map token) {
            ...
            try {
                ...
                String srcScript = String.format("syscxp-script-%s", UUID.randomUUID().toString());
                scriptFile = new File(PathUtil.join(PathUtil.getFolderUndersyscxpHomeFolder("temp-scripts"), srcScript));
                scriptContent = StringDSL.s(contents).formatByMap(token);

                String remoteScript = StringDSL.ln(
                        "/bin/bash << EOF",
                        "cat << EOF1 > {remotePath}",
                        "{scriptContent}",
                        "EOF1",
                        ...
                        "exit $ret",
                        "EOF"
                ).formatByMap(CollectionDSL.map(CollectionDSL.e("remotePath", String.format("/tmp/%s", UUID.randomUUID().toString())),
                        CollectionDSL.e("scriptContent", scriptContent),
                        CollectionDSL.e("parameters", parameters),
                        CollectionDSL.e("stdout", String.format("/tmp/%s", UUID.randomUUID().toString())),
                        CollectionDSL.e("stderr", String.format("/tmp/%s", UUID.randomUUID().toString()))
                ));
        ...
        ScriptRunner(String script) {
            String remoteScript = StringDSL.ln(
                    ...
                    CollectionDSL.e("scriptContent", script),
                    CollectionDSL.e("stdout", String.format("/tmp/%s", UUID.randomUUID().toString())),
                    CollectionDSL.e("stderr", String.format("/tmp/%s", UUID.randomUUID().toString()))
            ));
            scriptCommand = createCommand(remoteScript);
    ...
    public Ssh checkTool(String...toolNames) {
        ...
        String cmdstr = StringDSL.s("EXIT (){ echo \"$1\"; exit 1;}; cmds=\"{0}\"; for cmd in $cmds; do which $cmd >/dev/null 2>&1 || EXIT \"Not find command: $cmd\";  done").format(tool);
    ...
    private void build() throws IOException {
        ...
        if (privateKey != null) {
            privateKeyFile = File.createTempFile("syscxp", "tmp");
