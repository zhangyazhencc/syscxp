...
class SyscxpLibArgs(object):
    def __init__(self):
        self.syscxp_repo = None
        self.yum_server = None
        self.distro = None
        self.distro_version = None
        self.distro_release = None
        self.syscxp_root = None
...
def post_msg(msg, post_url):
    '''post message to syscxp, label for support i18n'''
...
def yum_enable_repo(name, enablerepo, host_post_info):
    ...
    syscxp_runner = SyscxpRunner(runner_args)
    result = syscxp_runner.run()
    logger.debug(result)
    if result['contacted'] == {}:
...
class SyscxpLib(object):
    def __init__(self, args):
        distro = args.distro
        distro_release = args.distro_release
        distro_version = args.distro_version
        syscxp_repo = args.syscxp_repo
        syscxp_root = args.syscxp_root
...
gpgcheck=0
            ...
            if syscxp_repo == "false":
                ...
                if distro_version >= 7:
                    copy_arg = CopyArg()
                    copy_arg.src = "files/syscxplib/syscxp-redhat.repo"
                    copy_arg.dest = "/etc/yum.repos.d/"
                    copy(copy_arg, host_post_info)
                # install epel-release
                if epel_repo_exist is False:
                    copy_arg = CopyArg()
                    copy_arg.src = "files/syscxplib/epel-release-source.repo"
                    copy_arg.dest = "/etc/yum.repos.d/"
                    copy(copy_arg, host_post_info)
                    # install epel-release
                ...
                if require_python_env == "true":
                    for pkg in ["python-devel", "python-setuptools", "python2-pip", "gcc", "autoconf", "ntp", "ntpdate"]:
                ...
                error("ERROR: Must syscxp_repo = false")
        else:
            error("ERROR: Unsupported distribution")

        if require_python_env == "true":
            # check the pip 9.0.1 exist in system to avoid site-packages enable potential issue
            ...
            if pip_match is False:
                ...
                host_post_info.post_label_param = syscxp_root
                run_remote_command("mkdir -p %s" % syscxp_root, host_post_info)
                # copy pip 9.0.1
                copy_arg = CopyArg()
                copy_arg.src = "files/pip-9.0.1.tar.gz"
                copy_arg.dest = "%s/pip-9.0.1.tar.gz" % syscxp_root
                copy(copy_arg, host_post_info)
                # install pip 9.0.1
                pip_install_arg = PipInstallArg()
                # pip_install_arg.extra_args = "\"--ignore-installed\""
                pip_install_arg.name = "%s/pip-9.0.1.tar.gz" % syscxp_root
