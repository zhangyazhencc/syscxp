...
@RestartableJob
@UniqueResourceJob
public class DeleteL3VpnJob implements Job {
    private static final CLogger LOGGER = Utils.getLogger(DeleteL3VpnJob.class);
    ...
    public void run(ReturnValueCompletion<Object> completion) {
        ...
        try {
            LOGGER.info("开始执行JOB【Delete L3Vpn】");
            ...
            bus.send(deleteVpnMsg, new CloudBusCallBack(completion) {
                ...
                public void run(MessageReply reply) {
                    if (reply.isSuccess()) {
                        LOGGER.debug(String.format("L3VPN[UUID:%s]删除成功", vpnUuid));
                        completion.success(null);
                    } else {
                        LOGGER.debug(String.format("L3VPN[UUID:%s]删除失败", vpnUuid));
    ...
    public static DeleteL3VpnJob executeJob(JobQueueFacade jobf, String vpnUuid, boolean deleteRenew){
        DeleteL3VpnJob job = new DeleteL3VpnJob();
        job.setVpnUuid(vpnUuid);
        job.setDeleteRenew(deleteRenew);
        jobf.execute("删除L3VPN", Platform.getManagementServerId(), job);
        return job;
    }
}
