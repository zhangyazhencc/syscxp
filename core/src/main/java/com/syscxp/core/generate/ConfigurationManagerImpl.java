...
public class ConfigurationManagerImpl extends AbstractService implements ConfigurationManager {
    ...
    private void handleApiMessage(APIMessage msg) {
        try {
             if (msg instanceof APIGenerateApiJsonTemplateMsg) {
    ...
    private void handle(APIGenerateApiTypeScriptDefinitionMsg msg) {
        ...
        List<Class> apiMsgClass = BeanUtils.scanClassByType("com.syscxp", APIMessage.class);
        List<Class> apiEventClass = BeanUtils.scanClassByType("com.syscxp", APIEvent.class);
        List<Class> apiReplyClass = BeanUtils.scanClassByType("com.syscxp", APIReply.class);
        List<Class> inventoryClass = BeanUtils.scanClass("com.syscxp", Inventory.class);
        ...
        String exportPath = msg.getOutputPath() != null ?
                msg.getOutputPath() : PathUtil.join(System.getProperty("user.home"), "syscxp-api-typescript", "api.ts");
    ...
    private void generateVOViewSql(StringBuilder sb, Class<?> entityClass) {
        ...
        sb.append(String.format("\nCREATE VIEW `syscxp`.`%s` AS SELECT ", entityClass.getSimpleName()));
        ...
        sb.append(String.format(" FROM `syscxp`.`%s` WHERE %s IS NULL;\n", EOClazz.getSimpleName(), at.softDeletedColumn()));
    }
    ...
    private void handle(APIGenerateSqlVOViewMsg msg) {
        try {
            ...
            String exportPath = PathUtil.join(System.getProperty("user.home"), "syscxp-mysql-view");
    ...
    private void generateGroovyClasses(APIGenerateGroovyClassMsg msg) {
        try {
            ...
            if (exportPath == null) {
                exportPath = PathUtil.join(System.getProperty("user.home"), "syscxp-groovy-template");
            }
    ...
    private void handle(APIGenerateTestLinkDocumentMsg msg) throws IOException {
        ...
        if (outputDir == null) {
            outputDir = PathUtil.join(System.getProperty("user.home"), "syscxp-testlink");
        }
    ...
    public void generateApiJsonTemplate(String exportPath, List<String> basePkgs) throws IOException {
        ...
        if (exportPath == null) {
            exportPath = PathUtil.join(System.getProperty("user.home"), "syscxp-python-template");
        }
        if (basePkgs == null || basePkgs.isEmpty()) {
    ...
    @Override
    public String getId() {
        return bus.makeLocalServiceId("generate");
    }
