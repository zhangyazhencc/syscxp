...
public class ApiTimeoutManagerImpl implements ApiTimeoutManager {
    ...
    class Value {
            ...
            for (String val : vals) {
                ...
                if (keyval.length != 2) {
                    throw new IllegalArgumentException(String.format("the key/value pair must be in format of key::value;" +
                            "invalid configuration[%s=%s] found, check your syscxp.properties", key, valueString));
                }
        ...
        public String getValue(String key) {
            ...
            if (val == null) {
                throw new IllegalArgumentException(String.format("not key[%s] found key/value pair[%s=%s], check your syscxp.properties",
