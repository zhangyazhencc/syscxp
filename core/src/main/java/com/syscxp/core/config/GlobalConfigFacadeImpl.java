...
public class GlobalConfigFacadeImpl extends AbstractService implements GlobalConfigFacade {
    ...
    private void handle(APIGetGlobalConfigMsg msg) {
        ...
        if (c == null) {
            ErrorCode err = Platform.argerr("unable to find GlobalConfig[category:%s, name:%s]", msg.getCategory(), msg.getName());
            reply.setError(err);
        } else {
    ...
    private void handle(APIUpdateGlobalConfigMsg msg) {
        ...
        try {
            ...
            evt.setError(Platform.argerr(e.getMessage()));
    ...
    public boolean start() {
        class GlobalConfigInitializer {
            ...
            private void parseConfig(File file) throws JAXBException, FileNotFoundException {
                ...
                com.syscxp.core.config.schema.GlobalConfig gb = (com.syscxp.core.config.schema.GlobalConfig) unmarshaller.unmarshal(file);
                for (com.syscxp.core.config.schema.GlobalConfig.Config c : gb.getConfig()) {
