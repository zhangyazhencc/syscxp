...
public aspect DeferAspect {
    ...
    pointcut deferCalled() : execution(@com.syscxp.core.defer.Deferred * *.*(..));
    
    declare error: call(void com.syscxp.core.defer.Defer.guard(..)) && !withincode(@com.syscxp.core.defer.Deferred * *.*(..)) : "Defer.guard() must be called in method with @Deferred annotation";
    declare error: call(void com.syscxp.core.defer.Defer.defer(..)) && !withincode(@com.syscxp.core.defer.Deferred * *.*(..)) : "Defer.defer() must be called in method with @Deferred annotation";
