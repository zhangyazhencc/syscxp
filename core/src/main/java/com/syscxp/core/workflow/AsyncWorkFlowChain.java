...
public class AsyncWorkFlowChain {
    ...
    private WorkFlowVO getFlowVOByPosition(int position) {
        ...
        query.add(WorkFlowVO_.position, SimpleQuery.Op.EQ, position);
        query.add(WorkFlowVO_.chainUuid, SimpleQuery.Op.EQ, chainvo.getUuid());
    ...
    public void rollback() {
        SimpleQuery<WorkFlowVO> query = dbf.createQuery(WorkFlowVO.class);
        query.add(WorkFlowVO_.chainUuid, SimpleQuery.Op.EQ, chainvo.getUuid());
        query.orderBy(WorkFlowVO_.position, SimpleQuery.Od.DESC);
    ...
    public void carryOn(String chainUuid, WorkFlowCallback callback) throws WorkFlowException {
        ...
        query.add(WorkFlowChainVO_.uuid, SimpleQuery.Op.EQ, chainUuid);
        chainvo = query.find();
        if (chainvo == null) {
    ...
    protected void carryOnRollback() {
        ...
        SimpleQuery<WorkFlowVO> query = dbf.createQuery(WorkFlowVO.class);
        query.add(WorkFlowVO_.chainUuid, SimpleQuery.Op.EQ, chainvo.getUuid());
        query.add(WorkFlowVO_.reason, SimpleQuery.Op.NOT_NULL);
